...
//  bool openFile( const char * path, const char * readType );
  ...
  uint32_t capacity() {
      ...
      // SD.cardSize() returns number of sectors, multiply by sector size
      uint64_t cardSizeBytes = (uint64_t)STORAGE_MANAGER.cardSize() * STORAGE_MANAGER.sectorSize();
      // Return in KB to avoid overflow (legacy FTP servers expect KB in some contexts)
      // But the callback expects bytes, so cap at uint32_t max if needed
      return (uint32_t)cardSizeBytes;
  };
  uint32_t free() {
      // Standard SD library doesn't expose FAT free space calculation
      // Return conservative estimate: card capacity minus 10% overhead
      // For accurate free space, SdFat library should be used instead
      uint64_t cardSizeBytes = (uint64_t)STORAGE_MANAGER.cardSize() * STORAGE_MANAGER.sectorSize();
      uint64_t estimatedFree = (cardSizeBytes * 9) / 10;  // 90% of capacity
